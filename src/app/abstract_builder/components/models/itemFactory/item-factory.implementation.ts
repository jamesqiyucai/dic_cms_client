...
export class ItemFactoryImplementation implements ItemFactory {
  ...
  public getSenseGroup(
    ...
    if (pos && inflections && phonetics && baseSenses && examples && stories) {
      return new SenseGroup(id, pos, phonetics, inflections, baseSenses, examples, stories);
    } else if (!pos && !phonetics && !inflections && !baseSenses && !examples && !stories) {
      return new SenseGroup(id, 1, List(), List(), List(), List(), List());
    } else {
