...
export class ProposalRepositoryImpl implements ProposalRepository {
  ...
  private _proposalDocumentHolders: ProposalDocumentHolder[];
  ...
  private getHolderByID(ID: number): ProposalDocumentHolder {
    ...
    holder.ID = ID;
    this._proposalDocumentHolders.push(holder);
    return holder;
  }
  public createBlankProposal(): ProposalEditorHandle {
  ...
  public loadPendingProposals() {
    ...
    const reviewer = this.userService.getCurrentUser().toString();
